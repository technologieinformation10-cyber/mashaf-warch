...
(() => {
  ...
  const el = {
    ...
    // ===== الواجهة الجديدة: التنقل السفلي والشاشات (إضافة) =====
    screenHome: document.getElementById("screenHome"),
    screenMemorize: document.getElementById("screenMemorize"),
    screenReview: document.getElementById("screenReview"),
    screenProgress: document.getElementById("screenProgress"),
    screenMore: document.getElementById("screenMore"),
    navHomeBtn: document.getElementById("navHomeBtn"),
    navMemorizeBtn: document.getElementById("navMemorizeBtn"),
    navReviewBtn: document.getElementById("navReviewBtn"),
    ...
    homeContinueSub: document.getElementById("homeContinueSub"),
    homeContinueBtn: document.getElementById("homeContinueBtn"),
    homeReviewTitle: document.getElementById("homeReviewTitle"),
    homeReviewSub: document.getElementById("homeReviewSub"),
    homeReviewBtn: document.getElementById("homeReviewBtn"),
    homeMemorizedPct: document.getElementById("homeMemorizedPct"),
    homeMemorizedBarFill: document.getElementById("homeMemorizedBarFill"),
    homeSolidPct: document.getElementById("homeSolidPct"),
    homeSolidBarFill: document.getElementById("homeSolidBarFill"),

    // شاشة الحفظ: لوحة "خيارات إضافية" القابلة للطي
    ...
    // شاشة المراجعة
    reviewPriorityList: document.getElementById("reviewPriorityList"),
    reviewStatusShortcutBtn: document.getElementById("reviewStatusShortcutBtn"),

    // شاشة التقدم
    progressMemorizedPct: document.getElementById("progressMemorizedPct"),
    progressMemorizedBarFill: document.getElementById("progressMemorizedBarFill"),
    progressSolidPct: document.getElementById("progressSolidPct"),
    progressSolidBarFill: document.getElementById("progressSolidBarFill"),
    progressStatsChips: document.getElementById("progressStatsChips"),

    // شاشة المزيد
    moreBrowsePageBtn: document.getElementById("moreBrowsePageBtn"),
    moreBrowseSurahBtn: document.getElementById("moreBrowseSurahBtn"),
    moreBrowseHizbBtn: document.getElementById("moreBrowseHizbBtn"),
    moreHizbTestShortcut: document.getElementById("moreHizbTestShortcut"),
    moreQuizShortcut: document.getElementById("moreQuizShortcut"),
    progressLogShortcut: document.getElementById("progressLogShortcut"),
    moreErrorStatsShortcut: document.getElementById("moreErrorStatsShortcut"),
    moreStatusShortcut: document.getElementById("moreStatusShortcut"),
    moreSettingsBtn: document.getElementById("moreSettingsBtn"),

    // نافذة الإعدادات وحول التطبيق
  ...
  // =========================================================================
  // ===== الواجهة الجديدة: التنقل السفلي + الشاشة الرئيسية + شاشتا المراجعة  =====
  // ===== والتقدم + شاشة المزيد + نافذة الإعدادات (إضافة عرض بحتة فقط —      =====
  // ===== تُعيد استخدام كل دوال الحفظ/التسجيل/الحساب الموجودة أعلاه حرفيًا،   =====
  // ===== ولا تُعدِّل أي منها ولا تحذف أي وظيفة قائمة)                        =====
  // =========================================================================

  const SCREENS = {
    home: { section: el.screenHome, nav: el.navHomeBtn },
    memorize: { section: el.screenMemorize, nav: el.navMemorizeBtn },
    review: { section: el.screenReview, nav: el.navReviewBtn },
    progress: { section: el.screenProgress, nav: el.navProgressBtn },
    more: { section: el.screenMore, nav: el.navMoreBtn },
  };
  ...
  function showScreen(name) {
    ...
    Object.keys(SCREENS).forEach((key) => {
      ...
      SCREENS[key].section.classList.toggle("screen-active", isActive);
      SCREENS[key].nav.classList.toggle("active", isActive);
      if (isActive) SCREENS[key].nav.setAttribute("aria-current", "page");
      else SCREENS[key].nav.removeAttribute("aria-current");
    });
    if (name === "home") renderHome();
    else if (name === "review") renderReviewSummary();
    else if (name === "progress") renderProgressSummary();
  }

  // نفس فلسفة الحماية المستخدمة أصلًا لأزرار تبديل الوضع أثناء التسجيل (تعطيلها
  // بدل تركها تقطع تسجيلًا جاريًا): يمكن البقاء في شاشة "الحفظ" أو العودة إليها
  // أثناء التسجيل، لكن لا يمكن مغادرتها إلى شاشة أخرى قبل إيقاف التسجيل.
  Object.keys(SCREENS).forEach((key) => {
    SCREENS[key].nav.addEventListener("click", () => {
      if (key !== "memorize" && isMainRecordingActive()) {
        showToast("أوقف التسجيل الحالي أولًا");
        return;
      }
      showScreen(key);
  ...
  // ----- الشاشة الرئيسية: بطاقتا "متابعة الحفظ" و"مراجعة اليوم" + شريطا التقدّم -----
  async function renderHome() {
    const type = state.recordTarget.type;
    const id = state.recordTarget.id;
    el.homeContinueTitle.textContent = targetLabel(type, id);
    try {
      const hasRec = type === "surah" ? await QuranDB.hasSurahRecording(id)
        : type === "hizb" ? await QuranDB.hasHizbRecording(id)
        : await QuranDB.hasRecording(id);
      el.homeContinueSub.textContent = hasRec
        ? "سجّلت هذا العنصر من قبل — استمع إليه أو أعد التسميع."
        : "لم تُسجَّل هذا العنصر بعد.";
    } catch (e) {
      el.homeContinueSub.textContent = "";
    }

    try {
      const [statusMap, hizbNums] = await Promise.all([computeHizbReviewStatus(), QuranDB.getAllHizbNumbers()]);
      const counts = { solid: 0, soon: 0, needs: 0, none: 0 };
      for (let h = 1; h <= 60; h++) counts[statusMap[h].level]++;
      const needsReview = counts.needs + counts.soon;
      el.homeReviewTitle.textContent = needsReview === 0
        ? (counts.none === 60 ? "ابدأ بتسميع أول حزب لك" : "كل الأحزاب النشطة ثابتة — أحسنت!")
        : `${needsReview} ${needsReview === 1 ? "حزب يحتاج" : "أحزاب تحتاج"} مراجعة الآن`;

      const memorizedPct = Math.round((hizbNums.length / 60) * 100);
      const solidPct = Math.round((counts.solid / 60) * 100);
      el.homeMemorizedPct.textContent = memorizedPct + "%";
      el.homeMemorizedBarFill.style.width = memorizedPct + "%";
      el.homeSolidPct.textContent = solidPct + "%";
      el.homeSolidBarFill.style.width = solidPct + "%";
    } catch (err) {
      el.homeReviewTitle.textContent = isStructuralDbError(err)
        ? "حدّث الصفحة لإصلاح قاعدة البيانات المحلية"
        : "تعذّر تحميل حالة المراجعة";
    }
  }
  el.homeContinueBtn.addEventListener("click", () => showScreen("memorize"));
  el.homeReviewBtn.addEventListener("click", () => showScreen("review"));

  // ----- شاشة المراجعة: قائمة مرتّبة حسب الأولوية، من نفس حساب computeHizbReviewStatus
  // المستخدم أصلًا في نافذة "حالة الحفظ" أعلاه، بلا أي تعديل عليه -----
  async function renderReviewSummary() {
    el.reviewPriorityList.innerHTML = '<p class="home-loading-note">جارٍ التحميل…</p>';
    try {
      const statusMap = await computeHizbReviewStatus();
      const counts = { solid: 0, soon: 0, needs: 0, none: 0 };
      for (let h = 1; h <= 60; h++) counts[statusMap[h].level]++;
      const rows = [
        { cls: "p-urgent", title: "يحتاج مراجعة عاجلة", n: counts.needs },
        { cls: "p-soon", title: "مراجعة قريبًا", n: counts.soon },
        { cls: "p-solid", title: "ثابت", n: counts.solid },
        { cls: "p-none", title: "لم يُسجَّل بعد", n: counts.none },
      ];
      el.reviewPriorityList.innerHTML = "";
      rows.forEach((r) => {
        if (r.n === 0) return;
        const btn = document.createElement("button");
        btn.type = "button";
        btn.className = `priority-row ${r.cls}`;
        btn.innerHTML =
          `<span class="priority-dot" aria-hidden="true"></span>` +
          `<span class="priority-row-text"><span class="priority-row-title">${r.title}</span>` +
          `<span class="priority-row-count">${r.n} من 60 حزبًا</span></span>` +
          `<span class="priority-row-chevron" aria-hidden="true">‹</span>`;
        // كل صف يفتح نفس نافذة "حالة الحفظ" التفصيلية الأصلية دون أي تكرار للمنطق
        btn.addEventListener("click", () => el.reminderBtn.click());
        el.reviewPriorityList.appendChild(btn);
      });
      if (!el.reviewPriorityList.children.length) {
        el.reviewPriorityList.innerHTML = '<p class="home-loading-note">لا توجد بيانات بعد.</p>';
      }
    } catch (err) {
      if (isStructuralDbError(err)) { renderDbRecoveryAction(el.reviewPriorityList, err); return; }
      el.reviewPriorityList.innerHTML = '<p class="home-loading-note">تعذّر تحميل حالة المراجعة</p>';
    }
  }
  el.reviewStatusShortcutBtn.addEventListener("click", () => el.reminderBtn.click());

  // ----- شاشة التقدم: شريطا نسبة مئوية + رقاقات ملخّص الحالة (نفس بيانات
  // statusSummaryRow الأصلية، معروضة هنا أيضًا بصيغة مختصرة) -----
  async function renderProgressSummary() {
    el.progressStatsChips.innerHTML = '<span class="status-chip">جارٍ التحميل…</span>';
    try {
      const [statusMap, hizbNums] = await Promise.all([computeHizbReviewStatus(), QuranDB.getAllHizbNumbers()]);
      const counts = { solid: 0, soon: 0, needs: 0, none: 0 };
      for (let h = 1; h <= 60; h++) counts[statusMap[h].level]++;
      const memorizedPct = Math.round((hizbNums.length / 60) * 100);
      const solidPct = Math.round((counts.solid / 60) * 100);
      el.progressMemorizedPct.textContent = memorizedPct + "%";
      el.progressMemorizedBarFill.style.width = memorizedPct + "%";
      el.progressSolidPct.textContent = solidPct + "%";
      el.progressSolidBarFill.style.width = solidPct + "%";

      el.progressStatsChips.innerHTML = "";
      [["solid", "ثابت"], ["soon", "يحتاج مراجعة قريبًا"], ["needs", "يحتاج مراجعة"], ["none", "لم يُسجَّل بعد"]].forEach(([level, label]) => {
        const chip = document.createElement("span");
        chip.className = `status-chip c-${level}`;
        chip.textContent = `${counts[level]} ${label}`;
        el.progressStatsChips.appendChild(chip);
      });
    } catch (err) {
      if (isStructuralDbError(err)) { renderDbRecoveryAction(el.progressStatsChips, err); return; }
      el.progressStatsChips.innerHTML = '<span class="status-chip">تعذّر التحميل</span>';
    }
  }

  // ----- شاشة الحفظ: لوحة "⚙️ خيارات إضافية" القابلة للطي (إضافة بصرية بحتة؛
  // كل عنصر بداخلها يبقى يعمل بمنطقه الأصلي كما هو تمامًا) -----
  el.advancedOptionsToggleBtn.addEventListener("click", () => {
    const willOpen = el.advancedOptionsPanel.classList.contains("collapsed");
    el.advancedOptionsPanel.classList.toggle("collapsed", !willOpen);
    el.advancedOptionsToggleBtn.setAttribute("aria-expanded", willOpen ? "true" : "false");
    el.advancedOptionsToggleBtn.classList.toggle("is-open", willOpen);
  });

  // ----- شاشة المزيد: اختصارات تصفح المصحف (تُفعِّل نفس أزرار تبديل الوضع الأصلية) -----
  el.moreBrowsePageBtn.addEventListener("click", () => { showScreen("memorize"); el.modePageBtn.click(); });
  el.moreBrowseSurahBtn.addEventListener("click", () => { showScreen("memorize"); el.modeSurahBtn.click(); });
  el.moreBrowseHizbBtn.addEventListener("click", () => { showScreen("memorize"); el.modeHizbBtn.click(); });

  // ----- شاشة المزيد: اختصارات إضافية تُطلق نفس الأزرار الأصلية حرفيًا (نفس المنطق تمامًا) -----
  el.moreHizbTestShortcut.addEventListener("click", () => el.hizbTestBtn.click());
  el.moreQuizShortcut.addEventListener("click", () => el.quizBtn.click());
  el.progressLogShortcut.addEventListener("click", () => el.progressLogBtn.click());
  el.moreErrorStatsShortcut.addEventListener("click", () => el.errorStatsBtn.click());
  el.moreStatusShortcut.addEventListener("click", () => el.reminderBtn.click());

  // ----- نافذة "الإعدادات وحول التطبيق" (جديدة بالكامل؛ إعادة الضبط تستخدم
  // نفس QuranDB.resetAll() ونفس showConfirm() الموجودتين أعلاه دون أي تعديل) -----
  function openSettingsModal() { el.settingsModal.classList.remove("hidden"); }
  el.settingsBtn.addEventListener("click", openSettingsModal);
  el.moreSettingsBtn.addEventListener("click", openSettingsModal);
  el.closeSettingsModal.addEventListener("click", () => el.settingsModal.classList.add("hidden"));
  ...
  el.settingsResetBtn.addEventListener("click", async () => {
    const ok = await showConfirm("سيؤدي هذا لحذف كل التسجيلات الصوتية وعلامات الأخطاء المحفوظة على هذا الجهاز نهائيًا، ولا يمكن التراجع عنه. هل تريد المتابعة؟");
    if (!ok) return;
    try {
      await QuranDB.resetAll();
      window.location.reload();
  ...
  // الشاشة الرئيسية معروضة افتراضيًا في HTML؛ هذا الاستدعاء يملأ محتواها
  // الديناميكي (بطاقتا المتابعة والمراجعة وشريطا التقدّم). state.recordTarget
  // مضبوطة بالفعل بشكل متزامن داخل loadPage/loadSurah/loadHizb أعلاه قبل أي
  // await فيها، فالقراءة هنا آمنة حتى قبل اكتمال تحميل التسجيل نفسه.
  showScreen("home");
})();
